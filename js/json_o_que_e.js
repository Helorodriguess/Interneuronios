const componentes = [
    ...
    },
    {
        id: 'meninge',
        nome: 'Meninge',
        texto: 'As meninges são membranas que se situam entre as estruturas que formam o sistema nervoso central e a camada óssea, estas três membranas são formadas de tecido conjuntivo e recebem o respectivos nomes dura-máter, aracnoide e pia-máter, elas são as responsáveis por  transportar substâncias importantes para o sistema nervoso. Entre estas meninges há o líquido cerebrospinal, uma solução salina, que possui função de proteção física e química, ou seja, quando ocorre um choque na cabeça, o líquido cerebrospinal deve ser comprimido antes que o encéfalo bata na parede interna do crânio, a fim de que tal líquido ajude a acolchoar o encéfalo.',
        imagens: [
            {
...
const div_o_que_e = document.getElementById('div_text');

for (let componente of componentes) {
    if (componente.id === comp) {

        titulo.innerHTML = `<h1>${componente.nome}</h1>`;

        div_o_que_e.innerHTML +=
            `<div class="titulo"> <img src="img/oQueEIcon.svg"/>${componente.nome}</div>
